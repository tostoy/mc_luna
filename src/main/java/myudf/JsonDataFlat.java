...
public class JsonDataFlat extends UDTF {
    ...
    public void process(Object[] args) throws UDFException {
        ...
        contentItems = contentItems.trim();
        externalAppendItems = externalAppendItems.trim();
        internalAppendItems = internalAppendItems.trim();
        ...
        */
        ...
        ReadContext ctx = JsonPath.using(conf).parse(jsonStr);
        ...
        for (int x = 0 ; x < contentResultList.get(0).size() ; x++ ) {
            ...
            for (int z = 0; z < contentNameList.size(); z++) {
                lineResult.put(contentNameList.get(z), contentResultList.get(z).get(x) == null ? "" : contentResultList.get(z).get(x).toString());
            }
